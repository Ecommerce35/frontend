...
const BrandProductList = () => {
  ...
  const [filters, setFilters] = useState({ color: [], size: [], vendor: [], rating: [] });
  const [pagination, setPagination] = useState({ next: null, previous: null }); // Pagination state
  const [filterOptions, setFilterOptions] = useState({ colors: [], sizes: [], vendors: [], brand: {} }); // Filter options
...
// // Function to update the URL parameters based on selected filters
// useEffect(() => {
...
//   // Update the URL with the new filters using navigate (instead of history.push)
            ...
            >
                <div className="container">
                  <h1 className="page-title">
                      {filterOptions.brand.title} <span>Shop</span>
                  </h1>
                </div>
                {/* End .container */}
            </div>
