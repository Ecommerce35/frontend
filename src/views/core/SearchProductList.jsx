...
const SearchProductList = () => {
  ...
  const [filters, setFilters] = useState({ color: [], size: [], vendor: [], brand: [], rating: [], category: [],  query: '', });
  const [pagination, setPagination] = useState({ next: null, previous: null }); // Pagination state
  const [filterOptions, setFilterOptions] = useState({ colors: [], sizes: [], brands: [], vendors: [], categories: [] }); // Filter options
  ...
  //   params.set('from', newValue[0]);
  //   params.set('to', newValue[1]);
  ...
  const handleSliderChange = (event, newValue, activeThumb) => {
    ...
    if (activeThumb === 0) {
      // Ensure the min point doesn't exceed the max point minus the minDistance
      const updatedValue = Math.min(newValue[0], selectedRange[1] - minDistance);
      setSelectedRange([updatedValue, selectedRange[1]]);
      
      // Update the URL with the new price range
      const params = new URLSearchParams(location.search);
      params.set('from', updatedValue);
      params.set('to', selectedRange[1]);
      navigate({ search: params.toString() }, { replace: true });
    } else {
      // Ensure the max point doesn't go below the min point plus the minDistance
      const updatedValue = Math.max(newValue[1], selectedRange[0] + minDistance);
      setSelectedRange([selectedRange[0], updatedValue]);
  
      // Update the URL with the new price range
      const params = new URLSearchParams(location.search);
      params.set('from', selectedRange[0]);
      params.set('to', updatedValue);
      navigate({ search: params.toString() }, { replace: true });
    }
  };
  ...
  const fetchProducts = async () => {
    const baseUrl = `/api/search/?q=${query}`;
    const params = new URLSearchParams(location.search);
    params.delete('q');  // Remove the query parameter from the URL search parameters

    const hasParams = Array.from(params.keys()).length > 0;

    // If there are additional parameters, append them to the base URL
    ...
    try {
        ...
        const { default_min, default_max } = data;

      // Set the price range and selected range for the slider
      setPriceRange([default_min, default_max]);
      setSelectedRange([min_price, max_price]);
    } catch (error) {
...
// }, [location.search]);
  ...
  const BASE_URL = " http://192.168.253.89:5173"; // Replace with your actual backend URL
...
const handleClearFilters = (event) => {
  ...
  setFilters({
      ...
      vendor: [],
      rating: [],
      query: filters.query, // Keep the current query
  });
  // Create a new URLSearchParams object
  const params = new URLSearchParams();
  // Append the 'q' parameter if it exists
  if (filters.query) {
      params.set('q', filters.query);
  }
  // Update the URL, keeping only the 'q' parameter
  navigate({
   ...
   useEffect(() => {
    ...
    setFilters({
      color: params.getAll('color').map(Number),
      size: params.getAll('size').map(Number),
      vendor: params.getAll('vendor').map(Number),
      brand: params.getAll('brand').map(Number),
      rating: params.getAll('rating').map(Number),
      query: params.get('q') || '',
    });

    const fromPrice = parseInt(params.get('from'), 10) || priceRange[0];
    const toPrice = parseInt(params.get('to'), 10) || priceRange[1];
    setSelectedRange([fromPrice, toPrice]);
  ...
  //       } else {
  //         console.warn(`Expected ids to be an array for filter type "${type}", but got`, ids);
  //       }
  //     }
  ...
  //     // Append the price range to the URL
  //     params.set('from', selectedRange[0]);
  //     params.set('to', selectedRange[1]);
  
  //     // Update the URL with the new filters and price range
  //     navigate({ search: params.toString() }, { replace: true });
  //   }, 300); // Debounce the URL update
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(() => {
      ...
      Object.entries(filters).forEach(([type, ids]) => {
        if (Array.isArray(ids)) ids.forEach(id => params.append(type, id));
      });
  ...
  }, [filters, selectedRange, navigate]);
                ...
                <div className="toolbox">
                    <div className="toolbox-left">
                        <a onClick={handleToggleSidebar} className="sidebar-toggler">
                            <MenuIcon fontSize='medium'/>Filters
                        </a>
                    </div>
                    {/* End .toolbox-left */}
                  ...
                  <div className="row">
                      {productCount > 0 ? (
                          productsWithRatings.map((p, index) => (
                              <div key={index} className="col-4 col-6 col-xxl-2 col-xl-3 col-sm-4 col-xs-6 col-lg-3 col-md-3 col-md-4 px-1">
                                  <div className="product">
                                      <figure className="product-media">
                                          <Link to={`/${p.product.slug}/${p.product.id}/${p.product.sub_category.slug}/`}>
                                              <img src={`${BASE_URL}${p.product.image}`} alt="Product image" className="product-image" />
                                          </Link>
